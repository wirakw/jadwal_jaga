	...
	public function send_mail()
{
	...
	$list_status = array();
	$tos = array();
	$from = new \SendGrid\Mail\From("kusumawira98@gmail.com", "Example User");
	$no=0;
	foreach ($data as $row) {
		...
		$tos[$no] = new \SendGrid\Mail\To(
				$list_mail[$no],
				"user".	$list_id[$no], array(
						'-name-' => $list_nama[$no],
						'-github-' => 'http://jj.opsigo.xyz/request/'.$list_id[$no]),
...
public function slack(){
    ...
    if ($this->slack->send('Jadwal Jaga Besok yaitu ')) {
